...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [bookingData, bookingServiceData, userData, packageData, serviceData] = await Promise.all([
  ...
  // Data for Charts
  const barChartData = {
    labels: ['Users', 'Packages', 'Services', 'Package Bookings', 'Service Bookings'],
    datasets: [
      {
        label: 'Count',
        data: [users.length, packages.length, services.length, bookings.length, bookingServices.length],
        backgroundColor: ['#8f2347', '#D98E73', '#C7596B', '#8F5C47', '#73D9C7'],
