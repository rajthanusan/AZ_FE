...
import { Container, Row, Col, Alert, Card as BootstrapCard } from 'react-bootstrap';
import { FaRegCalendarAlt, FaMapMarkerAlt, FaDollarSign, FaClock } from 'react-icons/fa';
  ...
  return (
    <section id="card" className="services">
      <Container className="my-1">
        ...
        {serviceError && <Alert variant="danger" className="error-alert">{serviceError}</Alert>}
        {serviceBookings.length === 0 ? (
          <p className="no-bookings">No service bookings found for your email.</p>
        ) : (
          <Row className="g-4">
            {serviceBookings.map(booking => (
              <Col sm={12} md={6} lg={4} key={booking._id}>
                <BootstrapCard className="booking-card mb-4 shadow-lg rounded-lg">
                  <BootstrapCard.Body>
                    <BootstrapCard.Title className="card-title">{booking.planName}</BootstrapCard.Title>
                    <div className="card-icon">
                      <FaMapMarkerAlt className="icon" />
                      <span className="card-text">{booking.serviceLocation}</span>
                    </div>
                    <div className="card-icon">
                    ...
                    <div className="card-icon">
                      ...
                      <span className="card-text">${booking.totalAmount ? booking.totalAmount.toFixed(2) : 'N/A'}</span>
                    </div>
                    <div className="card-icon">
                      <FaRegCalendarAlt className="icon" />
                      <span className="card-text">Booked On: {new Date(booking.createdAt).toLocaleString()}</span>
                    </div>
                  </BootstrapCard.Body>
                </BootstrapCard>
              </Col>
      ...
      <Container className="my-5">
        ...
        {planError && <Alert variant="danger" className="error-alert">{planError}</Alert>}
              ...
              <Col sm={12} md={6} lg={4} key={booking._id}>
                <BootstrapCard className="booking-card mb-4 shadow-lg rounded-lg">
                  <BootstrapCard.Body>
                    ...
                    <div className="card-icon">
                      <FaMapMarkerAlt className="icon" />
                      <span className="card-text">{booking.planserviceLocation}</span>
                    </div>
                    <div className="card-icon">
                      <FaClock className="icon" />
                      <span className="card-text">{booking.planduration} month(s)</span>
                    </div>
                    <div className="card-icon">
                      <FaDollarSign className="icon" />
                      <span className="card-text">${booking.plantotalAmount ? booking.plantotalAmount.toFixed(2) : 'N/A'}</span>
                    </div>
                    <div className="card-icon">
                      <FaRegCalendarAlt className="icon" />
                      <span className="card-text">Booked On: {new Date(booking.createdAt).toLocaleString()}</span>
