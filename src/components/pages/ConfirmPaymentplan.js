...
const ConfirmPaymentPlan = ({ planId, planType, planAmount, onClose }) => {
    ...
    try {
      ...
      for (const booking of existingBookings) {
        ...
        if (booking.planName === planType && currentDate.isBefore(endDate)) {
          toast.error(`You already have an active booking for the "${booking.planName}" plan at "${booking.planserviceLocation}". Please wait until ${endDate.format('MMMM Do YYYY')} to book again.`);
          resetForm(); // Reset form if an active booking is found
          return false;
        }
      }
    } catch (error) {
      console.error('Error checking existing bookings:', error.response ? error.response.data : error.message);
  ...
  return (
    <Container>
      <p><strong>Plan:</strong> {planType || 'Loading...'}</p>
      ...
      <Form>
        ...
        {!showPayPal && (
          <Button variant="primary" className="mt-3 custom-button " onClick={handleProceed} disabled={disableInputs}>
            Proceed to Payment
          </Button>
        )}
